...
from .uuidv7 import is_uuid_v7, is_canonical_lower_uuid
...
def derive_cursor(timestamp: int, event_id: str) -> str:
    ...
    plain = f"{timestamp}:{event_id}".encode("utf-8")
...
def decode_cursor(cursor: str) -> dict[str, Any]:
    ...
    Returns:
        Dict with 'timestamp' (int) and 'event_id' (str)
...
def assert_cursor_invariant(event: dict[str, Any]) -> CursorValidationResult:
    ...
    if not _is_valid_unix_seconds_int(ts) or not isinstance(eid, str) or not isinstance(cur, str):
