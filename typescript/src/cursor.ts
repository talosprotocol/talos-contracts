...
export function decodeCursor(cursor: string): DecodedCursor {
    ...
    if (colonIndex === -1) throw new Error("cursor frame must be '{timestamp}:{event_id}'");

    const tsStr = raw.slice(0, colonIndex);
    const eventId = raw.slice(colonIndex + 1);

    if (!isCanonicalTimestampString(tsStr)) throw new Error("timestamp is not canonical base-10");
    const tsNum = Number(tsStr);
    if (!isValidUnixSecondsInt(tsNum)) throw new Error("timestamp is out of range");

    if (!isUuidV7(eventId)) throw new Error("event_id is not uuidv7");
    if (!isCanonicalLowerUuid(eventId)) throw new Error("event_id must be lowercase canonical uuid");
...
export function assertCursorInvariant(event: {
    ...
    const derived = deriveCursor(event.timestamp, event.event_id);
    if (event.cursor !== derived) return { ok: false, derived, reason: "CURSOR_MISMATCH" };
