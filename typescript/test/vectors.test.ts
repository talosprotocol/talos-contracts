...
import { base64urlDecodeToBytes, base64urlEncodeBytes } from "../src/base64url.js";
...
const V = (name: string) => path.join(process.cwd(), "..", "test_vectors", name);

describe("base64url vectors", () => {
    test("valid cases", () => {
        const v = readJson<{ valid: { bytes_hex: string; encoded: string }[] }>(V("base64url.json"));
...
describe("cursor vectors", () => {
    test("deriveCursor matches goldens", () => {
        const v = readJson<{ timestamp: number; event_id: string; expected_cursor: string }[]>(V("cursor_derivation.json"));
        for (const c of v) {
